...
class Admin_Ajax
{
	...
	protected function update_theme_fonts($data)
	{
		...
		foreach (array('headlines', 'subtitles', 'paragraphs') as $font_type) {
			...
			if (isset($data['pwapp_edittheme_font'.$font_type])) {
				...
				if ($data['pwapp_edittheme_font'.$font_type] != Options::get_setting('font_'.$font_type)) {
		...
		if (isset($data['pwapp_edittheme_fontsize'])) {
			...
			if ($data['pwapp_edittheme_fontsize'] != Options::get_setting('font_size')) {
	...
	protected function resize_image($file_type, $file_path, $file_name, &$error_message = '')
	{
		...
		if (array_key_exists($file_type, Uploads::$allowed_files)) {
			...
			$arrMaximumSize = Uploads::$allowed_files[$file_type];
	...
	protected function remove_image($file_type)
	{
		...
		if ($previous_file_path != '') {
			$Uploads = $this->get_uploads_manager();
										...
										if (is_array($movefile)) {
											...
											if (isset($movefile['error'])) {
												...
												if ($copied_and_resized) {
													...
													// save option
													Options::update_settings($file_type, $uniqueFilename);
