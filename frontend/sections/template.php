<?php
	...
	$app_settings = Application::load_app_settings();
	...
	$texts_json_exists = Application::check_language_file( get_locale() );

	if ( false === $texts_json_exists ) {
		echo "ERROR, unable to load language file. Please check the '" . PWAPP_DOMAIN . "/frontend/locales/' folder.";
	}

	$background_color = Themes_Config::get_manifest_background( $app_settings['color_scheme'] );
	?>
<!DOCTYPE HTML>
...
<head>
	...
	<script type="text/javascript" pagespeed_no_defer="">
		var appticles = {
			...
			websiteUrl: '<?php echo home_url(); echo parse_url( home_url(), PHP_URL_QUERY ) ? '&' : '?'; echo Options::$prefix; ?>theme_mode=desktop',
